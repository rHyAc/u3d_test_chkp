...
class Demo():
    def __init__(self, args):
        ...
        ## NN models
        self.netD = EDDeconv(cin=3, cout=1, nf=64, zdim=512, activation=None)
        self.netA = EDDeconv(cin=3, cout=3, nf=64, zdim=512)
    ...
    def run(self, pil_im):
        ...
        with torch.no_grad():
            ...
            self.canon_depth_raw, self.d_msk, _, _, _, self.d_cf = self.netD(self.input_im)
            ...
            self.canon_albedo, self.a_msk, _, _, _, self.a_cf = self.netA(self.input_im)  # Bx3xHxW, ...
            ...
            self.recon_im = nn.functional.grid_sample(self.canon_im, grid_2d_from_canon, mode='bilinear', align_corners=True) 

            ## 90 degree
            
            yaw = self.renderer.render_yaw(self.canon_im, self.canon_depth, v_before=torch.tensor([-0.1*np.pi/180*60,0,0,0,0,-0.01]).unsqueeze(0).to(self.device), nsample=9, maxr=90)
            self.im_90 = yaw[:, 0]
            self.im_45 = yaw[:, 2]
            ...
            self.input_im = nn.functional.interpolate(self.input_im, (self.output_size, self.output_size), mode='bilinear', align_corners=False)
            ...
            self.im_90 = nn.functional.interpolate(self.im_90, (self.output_size, self.output_size), mode='bilinear', align_corners=False)
            self.im_45 = nn.functional.interpolate(self.im_45, (self.output_size, self.output_size), mode='bilinear', align_corners=False)
    ...
    def save_results(self, save_dir):
        def msk_sav_pre_proces(msk_to_proces, normalize=False):
            msk = []
            for mk in msk_to_proces:
                if normalize:
                    mk_f = mk.flatten(2)
                    mn = mk_f.min(2, True)[0].unsqueeze(3)
                    mx = mk_f.max(2, True)[0].unsqueeze(3)
                    mk = (mk - mn) / (mx - mn + EPS)
                    mk = mk.clamp(0, 1)
                msk.append(mk.detach().cpu().numpy())
            return msk
        ...
        save_image(save_dir, self.recon_im[0].clamp(-1,1)/2+0.5, 'rec_image')
        save_image(save_dir, self.recon_normal[0].permute(2,0,1)/2+0.5, 'rec_normal')

        save_image(save_dir, self.im_90[0].clamp(-1,1)/2+0.5, 'im_90')
        save_image(save_dir, self.im_45[0].clamp(-1,1)/2+0.5, 'im_45')
        ...
        d_msk = msk_sav_pre_proces(self.d_msk)
        save_image(save_dir, d_msk, 'D_MSK', msk=True, map=True)

        a_msk = msk_sav_pre_proces(self.a_msk)
        save_image(save_dir, a_msk, 'A_MSK', msk=True, map=True)

        d_cof = msk_sav_pre_proces(self.d_cf)
        save_image(save_dir, d_cof, 'D_COF', msk=True, map=True)  

        a_cof = msk_sav_pre_proces(self.a_cf)
        save_image(save_dir, a_cof, 'A_COF', msk=True, map=True)    
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', default='./demo/images/', type=str, help='Path to the directory containing input images')
    parser.add_argument('--result', default='./demo/results/', type=str, help='Path to the directory for saving results')
    parser.add_argument('--checkpoint', default='./ckp/checkpoint.pth', type=str, help='Path to the checkpoint file')
