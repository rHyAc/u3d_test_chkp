...
class Demo():
    def __init__(self, args):
        ...
        ## NN models
        self.netD = DANet(act=None)
        self.netA = DANet(odim=3)
    ...
    def load_checkpoint(self):
        ...
        self.netD.load_state_dict(cp['netD'], False)
        self.netA.load_state_dict(cp['netA'], False)
    ...
    def run(self, pil_im):
        ...
        with torch.no_grad():
            ...
            yaw = self.renderer.render_yaw(self.canon_im, self.canon_depth, v_before=torch.tensor([-np.pi/180*2,0,0,0,0,-0.01]).unsqueeze(0).to(self.device), nsample=9, maxr=90)
            self.im_90 = yaw[:, 0]
            self.im_45 = yaw[:, 2]
